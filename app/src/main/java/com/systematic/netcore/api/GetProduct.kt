...
class GetProduct {
    ...
    fun getProduct() {
        if (Common.isConnected(context)) {
            CallGetProduct().execute()
        } else {
            Common.showPoorConnectionToast(context)
        }
    }

    inner class CallGetProduct : AsyncTask<Void, Void, String>() {
...
*/
                ...
                Log.i(TAG, "resultForGetProduct= ${webServiceResult}")

                var jsonString = webServiceResult.toString()
                Log.i(TAG, "jsonStringForProduct = ${jsonString}")
                ...
                Log.i(TAG, "responseDataForProduct : $responseData")
                val productList = Gson().fromJson(responseData, Array<Product>::class.java)
                Log.i(TAG, "productList : ${productList.size}")

                if (productList.isNotEmpty()) {

                    val getProductListFromDb = database.getProductDAO().getProduct()

                    //database.getReceiptDAO().delete()

                    productList.forEach {
                        Log.i(TAG, "ProductName : ${it.ProductName}")

                        for (i in 0 until getProductListFromDb.size){
                            if (it.ProductID == getProductListFromDb[i].ProductID){
                                Log.i(TAG, "ProductID : ${it.ProductID}")
                                database.getProductDAO().deleteById(it.ProductID)
                                break
                            }
                        }

                        database.getProductDAO().insert(it)
                    }
                }
        ...
        override fun onPostExecute(result: String?) {
            ...
            if (response_info_code == "0") {
                ...
                val towerType = GetTowerType(activity!!, context!!)
