...
class GetCustomerGroup {
    ...
    fun getCustomerGroup() {
        if (Common.isConnected(context)) {
            CallGetCustomerGroup().execute()
        } else {
            Common.showPoorConnectionToast(context)
        }
    }

    inner class CallGetCustomerGroup : AsyncTask<Void, Void, String>() {
        ...
        override fun doInBackground(vararg params: Void?): String {
            try {
                ...
                Log.i(TAG, "resultForGetCustomerGroup= ${webServiceResult}")

                var jsonString = webServiceResult.toString()
                Log.i(TAG, "jsonStringForCustomerGroup = ${jsonString}")
                ...
                Log.i(TAG, "responseDataForCustomerGroup : $responseData")
                val customerGroupList = Gson().fromJson(responseData, Array<CustomerGroup>::class.java)
                Log.i(TAG, "customerGroupList : ${customerGroupList.size}")

                if (customerGroupList.isNotEmpty()) {

                    val getCustomerGroupListFromDb = database.getCustomerGroupDAO().getCustomerGroup()

                    //database.getReceiptDAO().delete()

                    customerGroupList.forEach {
                        Log.i(TAG, "name : ${it.CustomerGroupCode}")

                        for (i in 0 until getCustomerGroupListFromDb.size){
                            if (it.CustomerGroupID == getCustomerGroupListFromDb[i].CustomerGroupID){
                                Log.i(TAG, "CustomerID : ${it.CustomerGroupID}")
                                database.getCustomerGroupDAO().deleteById(it.CustomerGroupID)
                                break
                            }
                        }

                        database.getCustomerGroupDAO().insert(it)
                    }
                }
        ...
        override fun onPostExecute(result: String?) {
            ...
            if (response_info_code == "0") {
                ...
                val customer = GetCustomer(activity!!, context!!)
